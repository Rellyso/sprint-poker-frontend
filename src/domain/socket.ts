...
export interface ServerToClientEvents {
  ...
  // Eventos de erro
  error: (errorData: { code: string; message: string }) => void
}

// Definição dos tipos de eventos do cliente para o servidor
export interface ClientToServerEvents {
  // Eventos de sala
  '/room/join': (roomData: { roomId: string; userId: string }) => void
  '/room/leave': (roomData: { roomId: string; userId: string }) => void
  ...
  '/room/player/vote': (data: { roomId: string; vote: string | null }) => void
  '/room/select-story': (data: { storyId: string; roomId: string }) => void
  '/room/deselect-story': (data: { roomId: string }) => void
  '/room/story/submit-score': (data: {
    roomId: string
    storyId: string
    score: string
  }) => void
  '/room/votes/reset': (data: { roomId: string }) => void
}
